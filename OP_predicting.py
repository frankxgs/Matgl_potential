...
cache_2 = torch.load('data/op.pt')
non_nan_idx = (~torch.isnan(cache_1['labels']['OP'])).nonzero().squeeze()
ids = cache_2['ids']
Molecules = cache_2['Molecules']
Mols = []
for i in non_nan_idx:
    Mols.append(cache_1["Molecules"][i])
ops = cache_1['labels']['OP'][non_nan_idx].tolist()
...
with torch.no_grad():
    for mol in tqdm(Molecules):
        ...
        op_predictions.append(op_pred.to('cpu'))

op_scaled = np.array(op_predictions).reshape(-1, 1)
op_scaled = scaler.inverse_transform(op_scaled)
op_predictions = op_scaled.flatten().tolist()

predictions = {'id':ids, 'oxidation_potential':op_predictions}
df_pred = pd.DataFrame(predictions)
df_pred.to_csv('op_preds_1.csv', index=False)